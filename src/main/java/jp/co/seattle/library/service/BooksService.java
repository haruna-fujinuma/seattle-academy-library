...
public class BooksService {
	...
	public void updateBook(BookDetailsInfo bookInfo) {
		...
		if (bookInfo.getThumbnailUrl() == null) {
			...
			sql = "UPDATE books SET title = ?,  author = ?,  publisher = ?, publish_date = ?,  isbn = ?, description = ?, reg_date = NOW(), upd_date = NOW() WHERE books.id = ?";
			...
			// TODO 取得した書籍情報を更新するようにSQLを修正（タスク５）
			sql = "UPDATE books SET title = ?,  author = ?,  publisher = ?, publish_date = ?,  isbn = ?, description = ?, thumbnail_url = ?, thumbnail_name = ?, reg_date = NOW(), upd_date = NOW() WHERE books.id = ? ;";
			jdbcTemplate.update(sql, bookInfo.getTitle(), bookInfo.getAuthor(), bookInfo.getPublisher(),
